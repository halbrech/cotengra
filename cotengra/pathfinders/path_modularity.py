...
##########################################################
# stolen from https://gist.github.com/pszufe/02666497d2c138d1b2de5b7f67784d2b
##########################################################
...
def cnm_partition(
    ...
    H = [[] for _ in range(n)]
    for edge in hg.edges.values(): H[len(edge)].append(set(edge))
    partions = cnmAlgo(H, False, False)[1]
    print(partions)
    label = [None for _ in range(n)]
    for i, p in enumerate(partions): 
        for v in p: label[v] = i
    return label
    
labels_to_tree = PartitionTreeBuilder(cnm_partition)
